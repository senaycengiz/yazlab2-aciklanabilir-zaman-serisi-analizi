...
def save_skab_summary(rows):
    summary_path = f"{RESULTS_DIR}/skab_unseen_deep_learning_summary.csv"
...
def write_log(rows):
    log_path = f"{LOGS_DIR}/unseen_deep_learning_experiment.log"

    with open(log_path, "w") as file:
        file.write("Unseen Veri Senaryosu - Deep Learning Deney Logu\n")
        file.write("=" * 60 + "\n\n")
        file.write(
            "Not: Deep learning modelleri symbolic pattern/state kullanmadığı için "
            "unseen senaryosunda aynı test bölümü PCA tabanlı giriş olarak değerlendirilmiştir.\n"
        )
        file.write(
            "Unseen pattern üretimi results/unseen klasöründe ayrıca saklanmıştır.\n\n"
        )
...
def evaluate_skab():
    ...
    for fold in range(1, 6):
        ...
        for model_name, model_class, model_path_template in model_configs:
            ...
            print(f"SKAB Fold {fold} - {model_name} unseen veri testi başlatıldı.")
...
def evaluate_batadal():
    ...
    for model_name, model_class, model_path in model_configs:
        print(f"{dataset_name} - {model_name} unseen veri testi başlatıldı.")
...
if __name__ == "__main__":
    print("Unseen veri senaryosu deep learning değerlendirmesi başlatıldı.\n")

    skab_rows = evaluate_skab()
    batadal_rows = evaluate_batadal()

    save_csv(
        f"{RESULTS_DIR}/skab_unseen_deep_learning_results.csv",
        skab_rows
    )

    save_csv(
        f"{RESULTS_DIR}/batadal_unseen_deep_learning_results.csv",
        batadal_rows
    )
    ...
    print("\nUnseen deep learning değerlendirmesi tamamlandı.")
    print(f"SKAB sonuçları: {RESULTS_DIR}/skab_unseen_deep_learning_results.csv")
    print(f"SKAB özet sonuçları: {RESULTS_DIR}/skab_unseen_deep_learning_summary.csv")
    print(f"BATADAL sonuçları: {RESULTS_DIR}/batadal_unseen_deep_learning_results.csv")
    print(f"Log dosyası: {LOGS_DIR}/unseen_deep_learning_experiment.log")
